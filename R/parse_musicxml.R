...
parse_musicxml <- function(path) {
  ...
  doc <- read_xml(path)
  measures <- xml_find_all(doc, ".//part[1]/measure")
  ...
  for (measure_index in seq_along(measures)) {
    ...
    measure_number <- as.integer(xml_attr(measure_node, "number"))
    ...
    for (child in xml_children(measure_node)) {
      name <- xml_name(child)

      if (name == "attributes") {
        divisions_node <- xml_find_first(child, "./divisions")
        has_divisions <- !inherits(divisions_node, "xml_missing")

        if (has_divisions) {
          divisions <- as.numeric(xml_text(divisions_node))
        }
      ...
      if (name == "backup") {
        backup_duration_node <- xml_find_first(child, "./duration")
        backup_duration <- as.numeric(xml_text(backup_duration_node))
        position <- position - backup_duration
        next
      }

      if (name == "forward") {
        forward_duration_node <- xml_find_first(child, "./duration")
        forward_duration <- as.numeric(xml_text(forward_duration_node))
        position <- position + forward_duration
        next
      }

      if (name == "note") {
        duration_node <- xml_find_first(child, "./duration")
        duration <- as.numeric(xml_text(duration_node))
        chord_node <- xml_find_first(child, "./chord")
        rest_node <- xml_find_first(child, "./rest")
        ...
        alter_node <- xml_find_first(child, "./pitch/alter")

        alter <- "0"
        alter_exists <- !inherits(alter_node, "xml_missing")
        if (alter_exists) {
          alter <- xml_text(alter_node)
        }
        ...
        pitch <- if (is_rest) {
          ...
          step_node <- xml_find_first(child, "./pitch/step")
          octave_node <- xml_find_first(child, "./pitch/octave")
          step <- xml_text(step_node)
          octave <- xml_text(octave_node)
        ...
        staff_node <- xml_find_first(child, "./staff")
        staff <- as.integer(xml_text(staff_node))
